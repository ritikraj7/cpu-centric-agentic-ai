...
def is_smiles(text):
    ...
    try:
        # Suppress RDKit error messages when testing if text is SMILES
        with open(os.devnull, 'w') as devnull:
            with redirect_stderr(devnull), redirect_stdout(devnull):
                # Set RDKit verbosity to minimal to suppress C++ level messages
...
def pubchem_query2smiles(
    ...
    try:
        # Try both possible keys - PubChem sometimes returns "SMILES" instead of "IsomericSMILES"
        properties = data["PropertyTable"]["Properties"][0]
        if "IsomericSMILES" in properties:
            smi = properties["IsomericSMILES"]
        elif "SMILES" in properties:
            smi = properties["SMILES"]
        else:
            raise KeyError("Neither IsomericSMILES nor SMILES found in response")
    except KeyError:
